...
export default function MobileViewCard() {
    ...
    const Card = ({ item }) => (
        <div className="relative bg-white px-4 sm:px-6 py-6 sm:py-8 rounded-2xl shadow-lg transition-all duration-300 ease-in-out hover:shadow-xl w-full max-w-[320px] lg:max-w-[350px] h-auto">
            <Image
                src={item.image}
                alt={t(item.titleKey)}
                width={40}
            ...
            <p className="text-xs text-gray-600 text-center mt-2"> {t(item.descriptionKey)} </p>
        </div>
    );
    return (
        <div className="relative flex md:hidden flex-col items-center w-full lg:max-w-6xl mx-auto md:py-12"> {/* Top card */}
            <div className="w-full md:w-[230px] lg:w-full flex justify-center z-10 transition-opacity duration-700 px-4 sm:px-0">
                <Card item={topCard} />
            </div> {/* Bottom row */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-2 lg:gap-6 mt-8 sm:mt-12 w-full px-4 md:px-0">
